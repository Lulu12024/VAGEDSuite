...
namespace VAGSuite
{
    ...
    public partial class ctrlAirmassResult : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public Control PanelControls
        {
            get { return _panelControls; }
            set { _panelControls = value; }
        }
        private string _powerUnit = "bhp"; // Valeur par défaut
        public string PowerUnit
        {
            ...
            {
                _powerUnit = value;
                // Si on change l'unité, mettre à jour les étiquettes et la case à cocher kW
                if (_powerUnit == "kW")
        ...
        public PerformanceResults Calculate(string filename, SymbolCollection symbols)
        {
            ...
            try
            {
                ...
                // Vérifier si symbols est null ou vide
                if (symbols == null || symbols.Count == 0)
                {
                    throw new ArgumentException("La collection de symboles est nulle ou vide.");
                }

                SymbolHelper driverWishHelper = GetSymbolLike(symbols, "Driver wish", selectedBank);
                if (driverWishHelper == null)
                {
                    // Essayer des alternatives pour Driver wish
                    driverWishHelper = GetSymbolLike(symbols, "Pedal request", selectedBank);
                    if (driverWishHelper == null)
                        driverWishHelper = GetSymbolLike(symbols, "Torque request", selectedBank);
                    if (driverWishHelper == null)
                        throw new InvalidOperationException("Impossible de trouver le symbole 'Driver wish' ou équivalent dans la banque actuelle.");
                }


                int[] pedalrequestmap = readIntdatafromfile(filename, (int)driverWishHelper.Flash_start_address, driverWishHelper.Length);
                if (pedalrequestmap == null || pedalrequestmap.Length == 0)
                {
                    throw new InvalidOperationException("Échec de la lecture des données de la carte de demande de pédale depuis le fichier.");
                }

                limitermap = new int[pedalrequestmap.Length];
                int[] resulttable = new int[pedalrequestmap.Length]; // résultat 
                ...
                if (rows <= 0 || columns <= 0)
                {
                    throw new InvalidOperationException($"Dimensions de carte invalides: lignes={rows}, colonnes={columns}");
                }
                ...
                if (pedalXAxis == null || pedalXAxis.Length == 0)
                {
                    throw new InvalidOperationException("Échec de la lecture des données de l'axe X.");
                }

                if (pedalYAxis == null || pedalYAxis.Length == 0)
                {
                    throw new InvalidOperationException("Échec de la lecture des données de l'axe Y.");
                }
                ...
                for (int colcount = 0; colcount < columns; colcount++)
                {
                    for (int rowcount = 0; rowcount < rows; rowcount++)
                    {
                        try
                        {
                            // Obtenir la valeur actuelle de la carte de demande
                            int requestedValue = (int)pedalrequestmap.GetValue((colcount * rows) + rowcount);
                            limitType limiterType = limitType.None;

                            // Adapter le calcul en fonction du type d'ECU
                            int resultingAirMass;

                            if (_ECUType.Contains("EDC16"))
                            {
                                resultingAirMass = CalculateMaxAirmassforcell(symbols, filename,
                                    (int)pedalXAxis.GetValue(rowcount), ((int)pedalYAxis.GetValue(colcount)),
                                    requestedValue, checkEdit1.Checked, out limiterType, true);
                            }
                            else if (_ECUType.Contains("EDC17") || _ECUType.Contains("MED17"))
                            {
                                // Utiliser une méthode alternative pour EDC17/MED17
                                resultingAirMass = CalculateMaxAirmassforcellEDC17(symbols, filename,
                                    (int)pedalXAxis.GetValue(rowcount), ((int)pedalYAxis.GetValue(colcount)),
                                    requestedValue, checkEdit1.Checked, out limiterType);
                            }
                            else
                            {
                                // Méthode par défaut pour les autres types d'ECU
                                resultingAirMass = CalculateMaxAirmassforcell(symbols, filename,
                                    (int)pedalXAxis.GetValue(rowcount), ((int)pedalYAxis.GetValue(colcount)),
                                    requestedValue, checkEdit1.Checked, out limiterType, false);
                            }
                        ...
                        {
                            Console.WriteLine($"Erreur de traitement de la cellule [{rowcount},{colcount}]: {cellEx.Message}");
                            // Continuer avec la cellule suivante au lieu d'échouer tout le calcul
                            resulttable.SetValue(0, (colcount * rows) + rowcount);
                            limitermap.SetValue(limitType.None, (colcount * rows) + rowcount);
                        }
                    }
                }

                // Maintenant afficher resulttable
                ...
                // Maintenant remplir les lignes du tableau
                m_MaxValueInTable = 0;
                for (int r = 0; r < pedalXAxis.Length; r++)
                {
                    ...
                    for (int t = 0; t < pedalYAxis.Length; t++)
                    {
                        try
                        {
                            int index = (((t + 1) * rows) - 1) - r;
                            if (index >= 0 && index < resulttable.Length)
                            {
                                int currValue = (int)resulttable.GetValue(index);
                                if (currValue > m_MaxValueInTable) m_MaxValueInTable = currValue;
                                values.SetValue(currValue, t);
                            }
                            else
                            {
                                Console.WriteLine($"Index hors limites pour resulttable: {index}, longueur: {resulttable.Length}");
                                values.SetValue(0, t);
                            }
                        }
                        catch (Exception e)
                        {
                            Console.WriteLine($"Erreur de remplissage du tableau à la ligne {r}, colonne {t}: {e.Message}");
                            values.SetValue(0, t);
                        }
                    }
                    dt.Rows.Add(values);
                ...
                if (xtraTabControl1.SelectedTabPage != null && xtraTabControl1.SelectedTabPage.Name == xtraTabPage2.Name)
                {
                    LoadGraphWithDetails();
                }

                // Stockage pour les valeurs max
                double maxTorque = 0;
                    ...
                    for (int i = 0; i < dt.Rows[0].ItemArray.Length; i++)
                    {
                        try
                        {
                            ...
                            if (_ECUType.Contains("EDC16"))
                            {
                                ...
                                torque *= 10; // Correction pour garder le code identique à partir d'ici
                                double temptorque = torque * Tools.Instance.GetCorrectionFactorForRpm(rpm, m_numberCylinders);
                                torque = Convert.ToInt32(temptorque);
                            }

                            // Calcul de la puissance selon l'unité sélectionnée
                            int horsepower = Tools.Instance.TorqueToPower(torque, rpm);

                            // Convertir en ch si nécessaire
                            if (PowerUnit == "ch" && !checkEdit5.Checked)
                            {
                                // 1 bhp = 1.014 ch
                                horsepower = Convert.ToInt32(horsepower * 1.014);
                            }
                            else if (checkEdit5.Checked)
                            {
                                // Si l'utilisateur a coché la case pour afficher en kW
                                horsepower = Tools.Instance.TorqueToPowerkW(torque, rpm);
                            }

                            // Option pour afficher le couple en lb-ft si nécessaire
                            if (checkEdit6.Checked)
                            {
                                torque = Tools.Instance.IQToTorque(Convert.ToInt32(o), rpm, m_numberCylinders);
                            }

                            // Diviser par 100 pour obtenir les valeurs réelles
                            horsepower /= 100;
                            torque /= 100;
                        ...
                        {
                            Console.WriteLine($"Erreur de calcul de performance à l'index {i}: {calcEx.Message}");
                            // Continuer avec l'élément suivant au lieu d'échouer
                        }
                    }
                }
                else
                {
                    Console.WriteLine("La table de données est vide ou la première ligne n'a pas d'éléments.");
                }

                // Enregistrer les résultats
                pr.Torque = (int)maxTorque;
                pr.Horsepower = (int)maxHorsepower;
            }
            catch (Exception ex)
            {
                Console.WriteLine("Erreur de calcul de performance: " + ex.Message);
                Console.WriteLine("Trace de la pile: " + ex.StackTrace);

                // Afficher l'erreur à l'utilisateur
                DevExpress.XtraEditors.XtraMessageBox.Show(
                    "Échec du calcul des résultats de performance: " + ex.Message,
                    "Erreur de calcul",
                    MessageBoxButtons.OK,
                    MessageBoxIcon.Error);

                // Définir les valeurs à 0 pour indiquer l'échec du calcul
                pr.Torque = 0;
                pr.Horsepower = 0;
            }
        ...
        // Ajouter cette méthode de calcul spécifique pour EDC17
        private int CalculateMaxAirmassforcellEDC17(SymbolCollection symbols, string filename,
    int rpm, int load, int requestedValue, bool limitersEnabled, out limitType limiter)
        {
            limiter = limitType.None;
            int result = requestedValue;

            try
            {
                // Chercher des tables spécifiques à EDC17
                SymbolHelper torqueLimiter = GetSymbolLike(symbols, "Torque limitation", selectedBank);
                if (torqueLimiter == null)
                    torqueLimiter = GetSymbolLike(symbols, "Engine torque limiter", selectedBank);
                if (torqueLimiter == null)
                    torqueLimiter = GetSymbolLike(symbols, "Maximum torque", selectedBank);

                // Si un limiteur est trouvé et activé, appliquer la limitation
                if (torqueLimiter != null && limitersEnabled)
                {
                    // Lire la table de limitation de couple
                    int[] torqueLimiterMap = readIntdatafromfile(filename,
                        (int)torqueLimiter.Flash_start_address, torqueLimiter.Length);

                    // Obtenir les axes X et Y du limiteur
                    int[] torqueLimiterXAxis = readIntdatafromfile(filename,
                        torqueLimiter.Y_axis_address, torqueLimiter.Y_axis_length * 2);
                    int[] torqueLimiterYAxis = readIntdatafromfile(filename,
                        torqueLimiter.X_axis_address, torqueLimiter.X_axis_length * 2);

                    if (torqueLimiterMap != null && torqueLimiterXAxis != null && torqueLimiterYAxis != null)
                    {
                        // Trouver les indices les plus proches pour l'interpolation
                        int xIndex = FindClosestIndex(torqueLimiterXAxis, rpm);
                        int yIndex = FindClosestIndex(torqueLimiterYAxis, load);

                        // Pour simplifier, nous utilisons la valeur exacte sans interpolation
                        int mapIndex = (yIndex * torqueLimiter.Y_axis_length) + xIndex;

                        if (mapIndex >= 0 && mapIndex < torqueLimiterMap.Length)
                        {
                            int maxTorque = torqueLimiterMap[mapIndex];

                            // Si la valeur demandée dépasse la limite, appliquer la limitation
                            if (requestedValue > maxTorque)
                            {
                                result = maxTorque;
                                limiter = limitType.Torque;
                            }
                        }
                    }
                }

                // Chercher également un limiteur de fumée (Smoke limiter) pour EDC17
                SymbolHelper smokeLimiter = GetSymbolLike(symbols, "Smoke limiter", selectedBank);
                if (smokeLimiter == null)
                    smokeLimiter = GetSymbolLike(symbols, "Smoke limitation", selectedBank);
                if (smokeLimiter == null)
                    smokeLimiter = GetSymbolLike(symbols, "Maximum fuel quantity", selectedBank);

                // Appliquer le limiteur de fumée si trouvé et activé
                if (smokeLimiter != null && limitersEnabled)
                {
                    // Logique similaire à celle du limiteur de couple
                    int[] smokeLimiterMap = readIntdatafromfile(filename,
                        (int)smokeLimiter.Flash_start_address, smokeLimiter.Length);

                    int[] smokeLimiterXAxis = readIntdatafromfile(filename,
                        smokeLimiter.Y_axis_address, smokeLimiter.Y_axis_length * 2);
                    int[] smokeLimiterYAxis = readIntdatafromfile(filename,
                        smokeLimiter.X_axis_address, smokeLimiter.X_axis_length * 2);

                    if (smokeLimiterMap != null && smokeLimiterXAxis != null && smokeLimiterYAxis != null)
                    {
                        int xIndex = FindClosestIndex(smokeLimiterXAxis, rpm);
                        int yIndex = FindClosestIndex(smokeLimiterYAxis, load);

                        int mapIndex = (yIndex * smokeLimiter.Y_axis_length) + xIndex;

                        if (mapIndex >= 0 && mapIndex < smokeLimiterMap.Length)
                        {
                            int maxSmoke = smokeLimiterMap[mapIndex];

                            // Si la valeur limitée par le couple dépasse encore la limite de fumée
                            if (result > maxSmoke)
                            {
                                result = maxSmoke;
                                limiter = limitType.Smoke;
                            }
                        }
                    }
                }

                // Application de facteurs de correction spécifiques à EDC17
                // Ces facteurs peuvent varier selon le moteur/constructeur

                // 1. Correction en fonction de la température du moteur
                SymbolHelper engineTempCorrection = GetSymbolLike(symbols, "Engine temperature correction", selectedBank);
                if (engineTempCorrection != null)
                {
                    // On pourrait appliquer un facteur de correction, mais pour simplifier,
                    // nous supposons une température moteur optimale (pas de correction nécessaire)
                }

                // 2. Correction en fonction de la pression atmosphérique
                SymbolHelper pressureCorrection = GetSymbolLike(symbols, "Atmospheric pressure correction", selectedBank);
                if (pressureCorrection != null)
                {
                    // On suppose une pression atmosphérique normale (pas de correction)
                }

                // 3. Facteur de correction global pour EDC17
                // Certains EDC17 peuvent nécessiter un facteur d'échelle différent
                float correctionFactor = 1.0f;
                if (_ECUType.Contains("EDC17CP"))
                {
                    correctionFactor = 1.05f; // +5% pour certains EDC17CP
                }
                else if (_ECUType.Contains("EDC17C"))
                {
                    correctionFactor = 1.03f; // +3% pour certains EDC17C
                }

                // Appliquer le facteur de correction
                result = (int)(result * correctionFactor);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Erreur dans CalculateMaxAirmassforcellEDC17: {ex.Message}");
            }

            return result;
        }

        // Méthode auxiliaire pour trouver l'index le plus proche dans un tableau
        private int FindClosestIndex(int[] array, int value)
        {
            ...
            {
                int difference = Math.Abs(array[i] - value);
        ...
        public void AddPowerUnitToggleButton(DevExpress.XtraBars.Docking.DockPanel dockPanel)
        {
            // Vérifier si le bouton existe déjà pour éviter les doublons
            bool buttonExists = false;
            foreach (Control ctrl in dockPanel.Controls)
                {
                if (ctrl is DevExpress.XtraEditors.SimpleButton && ctrl.Name == "btnTogglePowerUnit")
                {
                    buttonExists = true;
                    break;
                }
            ...
            if (!buttonExists)
            {
                DevExpress.XtraEditors.SimpleButton btnTogglePowerUnit = new DevExpress.XtraEditors.SimpleButton();
                btnTogglePowerUnit.Name = "btnTogglePowerUnit";
                btnTogglePowerUnit.Text = "Toggle Power Unit";
                btnTogglePowerUnit.Width = 120;
                btnTogglePowerUnit.ToolTip = "Toggle between bhp/ch/kW";
                btnTogglePowerUnit.Click += (sender, e) => {
                    // Basculer entre les différentes unités de puissance
                    if (PowerUnit == "bhp")
                        PowerUnit = "ch";
                    else if (PowerUnit == "ch")
                        PowerUnit = "kW";
                    else
                        PowerUnit = "bhp";
                ...
                // Ajouter à votre interface (adaptez selon votre layout)
                dockPanel.Controls.Add(btnTogglePowerUnit);

                //panelControls.Controls.Add(btnTogglePowerUnit);
            }
        }

        // Méthode 5 (Alternative): RadioGroup pour choisir l'unité de puissance
        public void AddPowerUnitRadioGroup(Control containerPanel)
        {
            // Vérifier si le groupe radio existe déjà
            bool radioGroupExists = false;
            foreach (Control ctrl in containerPanel.Controls)
            {
                if (ctrl is DevExpress.XtraEditors.RadioGroup && ctrl.Name == "rgPowerUnit")
                {
            ...
            if (!radioGroupExists)
            {
                DevExpress.XtraEditors.RadioGroup rgPowerUnit = new DevExpress.XtraEditors.RadioGroup();
                rgPowerUnit.Name = "rgPowerUnit";
                rgPowerUnit.Properties.Items.Add(new DevExpress.XtraEditors.Controls.RadioGroupItem("bhp", "BHP"));
                rgPowerUnit.Properties.Items.Add(new DevExpress.XtraEditors.Controls.RadioGroupItem("ch", "CH"));
                rgPowerUnit.Properties.Items.Add(new DevExpress.XtraEditors.Controls.RadioGroupItem("kW", "kW"));

                // Sélectionner l'unité actuelle
                if (PowerUnit == "ch")
                    rgPowerUnit.SelectedIndex = 1;
                else if (PowerUnit == "kW" || checkEdit5.Checked)
                    rgPowerUnit.SelectedIndex = 2;
                else
                    rgPowerUnit.SelectedIndex = 0;

                rgPowerUnit.EditValueChanged += (sender, e) => {
                    PowerUnit = rgPowerUnit.EditValue.ToString();
                    // Mettre à jour l'interface
                    LoadGraphWithDetails();
                };

                // Ajouter à votre interface (adaptez selon votre layout)
                containerPanel.Controls.Add(rgPowerUnit);
            }
        }
        ...
        private void LoadGraphWithDetails()
        {
            if (gridControl1.DataSource != null)
            {
                ...
                // Définir les labels selon l'unité sélectionnée
                ...
                // Pour stocker les valeurs maximales
                double maxPower = 0;
                double maxTorque = 0;
                int maxPowerRpm = 0;
                int maxTorqueRpm = 0;
                ...
                for (int i = 0; i < dt.Rows[0].ItemArray.Length; i++)
                {
                    ...
                    // Calcul de base de la puissance
                    int horsepower = Tools.Instance.TorqueToPower(torque, rpm);

                    // Application des conversions selon les unités choisies
                    if (PowerUnit == "ch" && !checkEdit5.Checked)
                    {
                    ...
                    // Option pour le couple en lb-ft
                    if (checkEdit6.Checked)
                        torque = Tools.Instance.IQToTorque(Convert.ToInt32(o), rpm, m_numberCylinders);
                    ...
                    // Ajout des points aux séries du graphique
                    ...
                    // Capture des valeurs maximales et RPM correspondants
                ...
                // Afficher les valeurs maximales si vous avez des labels pour ça
                //if (lblMaxPower != null)
                //    lblMaxPower.Text = string.Format("Max: {0:F1} @ {1} rpm", maxPower, maxPowerRpm);

                //if (lblMaxTorque != null)
                //    lblMaxTorque.Text = string.Format("Max: {0:F1} @ {1} rpm", maxTorque, maxTorqueRpm);
            }
        }
